...
//show all titles
movies.forEach(m => {
    box.innerHTML += `<div class="text-center card p-3 m-3 w-50 mx-auto">${m.title}</div>`
})
...
movies2.forEach(m => {
    box.innerHTML += `<div class="text-center card p-3 m-3 w-50 mx-auto"><h2>${m.title}</h2><p>${m.description}</p> <img class="rounded w-50 mx-auto" src="images/star_wars/${m.poster}" alt="poster">
</div>`
})